...
export const GoToURLAction = new MultiCompBuilder(childrenMap, (props) => {
  return () => {
    const queryParams = keyValueListToSearchStr(props.query.map((i) => i.getView() as KeyValue));
    const hashParams = keyValueListToSearchStr(props.hash.map((i) => i.getView() as KeyValue));
